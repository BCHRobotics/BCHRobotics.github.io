<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Team 2386 Programming Hub</title>

  <!-- Pico CSS -->
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@picocss/pico@1/css/pico.min.css">

  <style>
    :root {
      --primary-blue: #0f2a44;
      --accent-gold: #e3b341;
      --secondary-blue: #1e4f7a;
      --card-bg: #ffffff;
      --bg-light: #f8f9fa;
    }

    body {
      background: var(--bg-light);
      color: #1f2933;
      font-family: 'Inter', sans-serif;
    }

    a {
      text-decoration: none;
    }

    /* ---------------- NAV ---------------- */
    nav {
      background: var(--primary-blue);
      border-bottom: 4px solid var(--accent-gold);
      padding: 0.5rem 1rem;
      display: flex;
      justify-content: space-between;
      align-items: center;
    }

    nav strong {
      font-size: 1.25rem;
      color: white;
    }

    nav ul {
      display: flex;
      list-style: none;
      gap: 1rem;
      margin: 0;
      padding: 0;
    }

    nav a {
      color: white;
      padding: 0.4rem 0.8rem;
      border-radius: 6px;
      font-weight: 600;
    }

    nav a[role="button"] {
      background: var(--accent-gold);
      color: var(--primary-blue);
    }

    nav a[role="button"]:hover {
      background: #cfa02f;
    }

    nav a:hover:not([role="button"]) {
      color: var(--accent-gold);
    }

    /* ---------------- HERO ---------------- */
    .hero {
      background: var(--primary-blue);
      color: white;
      padding: 4rem 1rem 3rem 1rem;
      text-align: center;
      position: relative;
      display: flex;
      flex-direction: column;
      align-items: center;
      gap: 1rem;
    }

    .hero img {
      max-width: 300px;
      height: auto;
    }

    .hero h1 {
      font-size: 2.5rem;
      margin: 0.5rem 0 0.3rem 0;
      color: var(--accent-gold);
      text-shadow: 2px 2px 6px rgba(0,0,0,0.4);
    }

    .hero p {
      font-size: 1.2rem;
      margin: 0;
      color: white;
      text-shadow: 1px 1px 4px rgba(0,0,0,0.4);
    }

    @media (max-width: 768px) {
      .hero h1 {
        font-size: 2rem;
      }
      .hero p {
        font-size: 1rem;
      }
      .hero img {
        max-width: 220px;
      }
    }

    /* ---------------- REPOS ---------------- */
    #repos {
      padding: 4rem 1rem;
      max-width: 1200px;
      margin: 0 auto;
    }

    #repos h2 {
      color: var(--primary-blue);
      border-bottom: 4px solid var(--accent-gold);
      padding-bottom: 0.5rem;
      font-size: 2rem;
      margin-bottom: 2rem;
      text-align: center;
    }

    .grid {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
      gap: 2rem;
    }

    .repo-card {
      background: linear-gradient(135deg, var(--card-bg), #f1f3f5);
      border-radius: 1rem;
      padding: 1.5rem;
      text-align: center;
      box-shadow: 0 6px 20px rgba(0,0,0,0.1);
      transition: transform 0.3s ease, box-shadow 0.3s ease;
      border-top: 5px solid var(--accent-gold);
    }

    .repo-card:hover {
      transform: translateY(-6px);
      box-shadow: 0 14px 30px rgba(0,0,0,0.2);
    }

    .repo-card h3 {
      color: var(--secondary-blue);
      margin-bottom: 1rem;
      font-size: 1.5rem;
    }

    .repo-card img {
      max-width: 180px;
      margin: 0.8rem 0;
      border-radius: 0.5rem;
      transition: transform 0.3s ease;
    }

    .repo-card img:hover {
      transform: scale(1.05);
    }

    .repo-card a {
      display: inline-block;
      margin-top: 0.8rem;
      padding: 0.5rem 1rem;
      background: var(--primary-blue);
      color: white;
      font-weight: 600;
      border-radius: 6px;
      transition: background 0.3s ease;
    }

    .repo-card a:hover {
      background: var(--accent-gold);
      color: var(--primary-blue);
    }

    /* ---------------- LINKS ---------------- */
    #links {
      padding: 4rem 1rem;
      text-align: center;
    }

    #links h2 {
      color: var(--primary-blue);
      border-bottom: 4px solid var(--accent-gold);
      font-size: 2rem;
      padding-bottom: 0.5rem;
      margin-bottom: 2rem;
    }

    #links ul {
      list-style: none;
      padding: 0;
      margin: 0 auto;
      display: flex;
      flex-direction: column;
      gap: 1rem;
      max-width: 400px;
    }

    #links a {
      font-weight: 600;
      color: var(--secondary-blue);
      padding: 0.4rem;
      transition: color 0.3s ease;
    }

    #links a:hover {
      color: var(--accent-gold);
    }

    /* ---------------- FOOTER ---------------- */
    footer {
      text-align: center;
      padding: 2rem 1rem;
      background: var(--primary-blue);
      color: white;
    }

  </style>
</head>

<body>

  <!-- NAV -->
  <nav class="container-fluid">
    <strong>Team 2386 Programming</strong>
    <ul>
      <li><a href="#repos">Documentation</a></li>
      <li><a href="#repos">Repositories</a></li>
      <li><a href="#links">Links</a></li>
      <li><a href="https://bchrobotics.com/" role="button">Main Site</a></li>
    </ul>
  </nav>

  <!-- HERO -->
  <section class="hero">
    <img src="https://raw.githubusercontent.com/BCHRobotics/BCHRobotics.github.io/refs/heads/master/mkdocs-windmill-dark/mkdocs_windmill_dark/img/trojan_title.png" alt="Team 2386 Trojan Logo">
    <h1>Team 2386 Programming Hub</h1>
    <p>Official programming documentation hub for FRC Team 2386.</p>
  </section>

  <!-- REPOSITORIES -->
  <section id="repos">
    <h2>Repositories</h2>
    <div class="grid">

      <article class="repo-card">
        <h3>TankBot</h3>
        <img src="img/tankbot.png" alt="TankBot">
        <a href="https://github.com/BCHRobotics/TankBot" target="_blank">View Repository →</a>
      </article>

      <article class="repo-card">
        <h3>MiniBot</h3>
        <img src="img/minibot.png" alt="MiniBot">
        <a href="https://github.com/BCHRobotics/MiniBot" target="_blank">View Repository →</a>
      </article>

      <article class="repo-card">
        <h3>2025 Competition Robot</h3>
        <img src="img/2025.png" alt="2025 Robot">
        <a href="https://github.com/BCHRobotics/2025_Updated" target="_blank">View Repository →</a>
      </article>

      <article class="repo-card">
        <h3>2024 Competition Robot</h3>
        <img src="img/2024.png" alt="2024 Robot">
        <a href="https://github.com/BCHRobotics/2024_Updated" target="_blank">View Repository →</a>
      </article>

    </div>
  </section>

  <!-- LINKS -->
  <section id="links">
    <h2>Other Important Links</h2>
    <ul>
      <li><a href="https://github.com/BCHRobotics/BCHRobotics.github.io" target="_blank">Website GitHub Repository</a></li>
      <li><a href="https://github.com/BCHRobotics" target="_blank">FRC 2386 GitHub Organization</a></li>
      <li><a href="https://bchrobotics.com/" target="_blank">Main Team Website</a></li>
      <li><a href="https://github.com/full-auto-robots" target="_blank">FAR Robotics GitHub</a></li>
    </ul>
  </section>

  <!-- FOOTER -->
  <footer>
    © Team 2386 • Programming Documentation Hub
  </footer>

</body>
</html>
